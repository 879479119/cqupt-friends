<template>
  <div>
    <h2>学院列表</h2>
    <div class="container">
      <el-card :body-style="{ padding: '0px' }" v-for="(o, index) in departList" v-bind:key="index">
        <a :style="{backgroundImage: `url(${imgs[index]})`}" :href="'#/specList/'+o.depart">
          <span>{{o.depart}}</span>
        </a>
      </el-card>
    </div>
